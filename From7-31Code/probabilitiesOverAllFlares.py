...
def probabilities(hxrintlist,flarelist,peakslist,eRange,peakInt,units):
    ...
    for i in tqdm(range(len(hxrintTest))):
        ...
        '''if xCount/allCount > 0.8 and done == False:
            done = True
    ...
    plt.axvline(x=1.18*10**4, color='r')
    plt.xlim(minHxr,maxHxr)
    plt.xscale('log')
    plt.legend()
    plt.title(f'Class Distribution Given HXR {peakInt}')
    plt.xlabel(f'Lower Bound of HXR {peakInt} ({eRange} keV) ({units})')
    ...
    plt.xscale('log')
    plt.legend()
    plt.title(f'Flares Included By Class Given HXR {peakInt}')
    plt.xlabel(f'Lower Bound of HXR {peakInt} ({eRange} keV) ({units})')
    plt.ylabel('Percent')
...
'''
probabilities(ints410,flareClass,peaks410,'4-10','Integral','Counts')
probabilities(ints1015,flareClass,peaks1015,'10-15','Integral','Counts')
'''
probabilities(ints1525,flareClass,peaks1525,'15-25','Integral','Counts')
exit()
'''
probabilities(ints2550,flareClass,peaks2550,'25-50','Integral','Counts')
'''
probabilities(ints5084,flareClass,peaks5084,'50-84','Integral','Counts')
'''
exit()
