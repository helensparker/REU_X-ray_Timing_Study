...
for i in tqdm(range(9782)):
    ...
    for j in tqdm(range(index,25218)):
        ...
        if abs(startdif.total_seconds()) < 1800 and dgstart < dsend and dsstart < dgend and visFromEarth:
            matchingFlares.append([df.iloc[i,0],df.iloc[i,1],df2.iloc[j,0],df2.iloc[j,1],df2.iloc[j,9],df2.iloc[j,10],df2.iloc[j,11],df2.iloc[j,12],df2.iloc[j,13]])
            index = j+1
            break
with open('CandAboveVisfromEarthBackgroundmatching_flares.csv', 'w', newline='') as file:
