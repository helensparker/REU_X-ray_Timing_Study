...
df = pd.read_csv('LTCMorePeaksFixLowFlaresRemoveuselesspeaktimesandVals.csv')

data = []

for i in tqdm(range(0,4378)):
    start = df.iloc[i,0]
    end = df.iloc[i,1]
    time18 = df.iloc[i,2]
    val18 = df.iloc[i,3]
...
with open('LTCMorePeaksFixLowFlaresRemoveuselesstimeDifsPeaktimesandVals.csv', 'w', newline='') as file :
