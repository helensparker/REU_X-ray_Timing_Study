...
def histogram(flarelist, timediflist, title, xlab, saveas):
    ...
    labels = [f'C Class: {len(data_by_class['C'])} flares, Mean: {round(sum(data_by_class['C'])/len(data_by_class['C']))}', f'M Class: {len(data_by_class['M'])} flares, Mean: {round(sum(data_by_class['M'])/len(data_by_class['M']))}', f'X Class: {len(data_by_class['X'])} flares, Mean: {round(sum(data_by_class['X'])/len(data_by_class['X']))}']
    plt.hist(data, bins=50, stacked=True, color=colors, label=labels)
    plt.title(title+f': {len(timediflist)} flares, Mean: {round(sum(timediflist)/len(timediflist))}')
    plt.xlabel(xlab)
    plt.ylabel('Flares')
    plt.legend()
...
aotitle = 'Attenuator Corrected - Original Peak Times'
astitle = 'Attenuator Corrected - Smooth Peak Times'
