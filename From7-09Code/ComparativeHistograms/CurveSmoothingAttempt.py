...
for i in tqdm(range(index,index+distance)):
    ...
    for j in range(16,4,-1):
        ...
        if not type(goes) is list:
            ...
            smooth0 = smoothCurve(0,dsfakestart,dgend,df.iloc[i,21],df.iloc[i,5])
            smooth1 = smoothCurve(1,dsfakestart,dgend,df.iloc[i,22],df.iloc[i,7],)
            smooth2 = smoothCurve(2,dsfakestart,dgend,df.iloc[i,23],df.iloc[i,9])
            smooth3 = smoothCurve(3,dsfakestart,dgend,df.iloc[i,24],df.iloc[i,11])
            smooth4 = smoothCurve(4,dsfakestart,dgend,df.iloc[i,25],df.iloc[i,13])
            ...
            plt.savefig(f'{i}SmoothedCurves{j}.png')
            break