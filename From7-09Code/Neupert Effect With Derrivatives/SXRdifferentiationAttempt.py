...
def nonSmoothedDifferentiation(array):
    valsList = []
    timesList = []
    FulltimesList = []
    for k in range(len(array)):
        valsList.append(array[k][1])
        timesList.append(k)
    for i in range(len(timesList)):
        FulltimesList.append(dgstart + timedelta(0,timesList[i]))
    return FulltimesList,differentiate(timesList,valsList)

def smoothDeriv(deriv):
    if type(deriv) == int:
        return deriv
    smderiv = np.convolve(deriv,np.ones(61)/61,mode='same')
    return smderiv

def fullDifferentiation(array):
    FulltimesList, deri = nonSmoothedDifferentiation(array)
    smderiv = smoothDeriv(deri)
    return FulltimesList,smderiv
...
for i in tqdm(range(index,index+distance)):
    ...
    for j in range(16,4,-1):
        ...
        if not type(goes) is list:
            ...
            ax1.plot(smooth0[0],correctedLC0,label = '4-10keV')
            ax1.plot(smooth1[0],correctedLC1,label = '10-15keV')
            ax1.plot(smooth2[0],correctedLC2,label = '15-25keV')
            ax1.plot(smooth3[0],correctedLC3,label = '25-50keV')
            ax1.plot(smooth4[0],correctedLC4,label = '50-84keV')
            ax1.legend()
            ...
            times,sxrVals = fullDifferentiation(arrgoes)

            ax1 = lc.peek()

            ax2 = ax1.twinx()
            #goes.plot(axes=ax2)
            ax2.plot(times,sxrVals,label = '1.0 - 8.0 Å', color = 'red')
            ax2.set_ylabel(r'Watts m$^{-2}$ s$^{-1}$')
            ax2.legend()
