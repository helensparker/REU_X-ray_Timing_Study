...
with open('peaksAndSuch.csv', 'a', newline='') as file :
    writer = csv.writer(file)

    for j in range(10):

        triggerNum = df.iloc[j,0]
        print(triggerNum)
        ...
        allPeak = findPeak(timeList,sumList)[1]
        smAllPeak = findPeak(timeList,smoothedSumList)[1]
        ntPeak = findPeak(timeList,nonThermalSumList)[1]
        smNtPeak = findPeak(timeList,ntsmoothedSumList)[1]

        allPeak += triggerTime
        smAllPeak += triggerTime
        ntPeak += triggerTime
        smNtPeak += triggerTime


        realRow = df.iloc[j].tolist()
        print(realRow)
        realRow.extend([allPeak,ntPeak,smAllPeak,smNtPeak])
