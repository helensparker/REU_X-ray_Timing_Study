...
df = pd.read_csv('LTCintegrateHXR.csv')
df2 = pd.read_csv('LTCDifsClasses.csv')

index = 0
fullList = []
for i in tqdm(range(4370)):
    start = df2.iloc[i,0]
    end = df2.iloc[i,1]
    for j in tqdm(range(index,4453)):
        sstart = df.iloc[j,0]
        send = df.iloc[j,1]
        ...
        if start == sstart:
            ...
            index = j+1
            break
with open('LTCDifsClassesIntegrals.csv', 'w', newline='') as file:
