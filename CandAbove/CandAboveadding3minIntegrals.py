...
df = pd.read_csv('CandAboveintegrateHXR.csv')
df2 = pd.read_csv('CandAboveDifsClsasses.csv')

index = 0
fullList = []
for i in tqdm(range(4372)):
    start = df2.iloc[i,0]
    end = df2.iloc[i,1]
    for j in tqdm(range(index,4506)):
        sstart = df.iloc[j,0]
        send = df.iloc[j,1]
        ...
        if startdif.total_seconds() < -86400:
            break
        if start == sstart:
            fullList.append([df2.iloc[i,0],df2.iloc[i,1],df2.iloc[i,2],df2.iloc[i,3],df2.iloc[i,4],df2.iloc[i,5],df2.iloc[i,6],df2.iloc[i,7],df2.iloc[i,8],df2.iloc[i,9],df2.iloc[i,10],df2.iloc[i,11],df2.iloc[i,12],df2.iloc[i,13],df2.iloc[i,14],df2.iloc[i,15],df2.iloc[i,16],df2.iloc[i,17],df2.iloc[i,18],df2.iloc[i,19],df2.iloc[i,20],df2.iloc[i,21],df.iloc[j,2],df.iloc[j,3],df.iloc[j,4],df.iloc[j,5],df.iloc[j,6]])
            index = j+1
            break
with open('CandAboveDifsClsassesIntegrals.csv', 'w', newline='') as file:
