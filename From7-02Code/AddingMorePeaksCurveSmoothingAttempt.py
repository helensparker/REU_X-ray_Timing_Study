...
def findPeak(timeslist,smoothedVals,start):
    max = 0
    times = 0
...
index = 3249
distance = 4293-index+2
with open('LTCMPFLFNoAttFixPeaks.csv', 'a', newline='') as file :
    ...
    for i in tqdm(range(index,index+distance)):
        ...
        for j in range(16,4,-1):
            ...
            if not type(goes) is list:
                ...
                peak0, time0, dif0 = findPeak(smooth0[0],smooth0[1],start)
                peak1, time1, dif1 = findPeak(smooth1[0],smooth1[1],start)
                peak2, time2, dif2 = findPeak(smooth2[0],smooth2[1],start)
                peak3, time3, dif3 = findPeak(smooth3[0],smooth3[1],start)
                peak4, time4, dif4 = findPeak(smooth4[0],smooth4[1],start)

                realRow = df.iloc[i].tolist()
                realRow.extend([time0, dif0, peak0, time1, dif1, peak1, time2, dif2, peak2, time3, dif3, peak3, time4, dif4, peak4])
